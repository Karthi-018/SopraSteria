...
<center><%@include file= "cusmenu.jsp" %></center>
 <h3 style = "text-align:center;">Viewing your Wishlist Products</h3>
...
<%   pnames = (ArrayList<String>)request.getAttribute("pnames");%>
  ...
  <thead>
   ...
   for(int i=0;i<pids.size();i++){
	   ...
	   out.println("<form action = 'movecart'>");
	   out.println("<button name = 'pid' value = " + "'"+ pids.get(i)+"'"+ ">Move To Cart</button><br></form>");
	   out.println("<form action = 'deletewishlist'>");
	   out.println("<button name = 'pid' value = " + "'"+ pids.get(i)+"'"+ ">Remove</button><br></form></td>");
	   out.println("<tr>");
   }
   %>
  </tbody>

...
<td><%=s.getDescp()%></td>
<td><%=s.getPrice()%></td>
<td><form action='ViewAddToCart'><button name='id' value='<%= s.getPid()%>'>Add To Cart</button></form>
<td><form action='ViewWishList'><button name='id' value='<%= s.getPid()%>'>WishList</button></form>
<td><form action='ViewBuy'><button name='id' value='<%= s.getPid()%>'>Buy</button></form>
</tr>
</tbody>
	<%  }
		} %> 
